function verCursosAsistencias(nombreSeccion, tipo) {
  ...
  const opcionesGenerales = [
    {
      nombre: "Asistencias Generales",
      docente: "Registro General",
      img: "img/cursos/imagenCursos.png",
      onClick: `verAsistenciasGeneralesSeccion('${nombreSeccion}', '${tipo}')`,
    },
  ];
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ${opcionesGenerales
        .map(
          ...
          <div class="bg-white shadow-md rounded-xl p-5 flex flex-col items-center justify-center cursor-pointer hover:shadow-lg hover:scale-105 transition-transform duration-300"
               onclick="${opt.onClick}">
            <img src="${opt.img}" alt="${opt.nombre}" class="w-24 h-24 mb-3 rounded-full object-cover border-2 border-green-200" />
      ...
      ${cursos
        .map(
          (c) => `
          <div class="bg-white shadow-md rounded-xl p-5 flex flex-col items-center justify-center cursor-pointer hover:shadow-lg hover:scale-105 transition-transform duration-300"
               onclick="verDetalleCursoAsistencias('${nombreSeccion}', '${c.nombre}', '${tipo}')">
...
function verAsistenciasGeneralesSeccion(nombreSeccion, tipo) {
  const contenido = document.getElementById("contenido");
